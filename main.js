...
if(buffStr[buffStr.length-1]==='+/–'){doKey('+/–',true);buffStr.pop();}
...
function kp(e)
{var key=e.which,keyMatch=(',|.|-|–|/|÷|*|×|#|+/–|x|x!|E|EE|e|ex| |=|r|x√y|R|√|p|π|^|yx|\'|yx|"|yx|m|mr|v|mc|b|m+|n|m-|'+
's|sin|c|C|tan|S|sin-1|C|cos-1|T|tan-1|d|Deg|°|Deg|l|ln|L|log|\\|1/x|X|2x').split('|'),keyMatchHold=('sin|c|sinh|cosh|tan|tanh|m-|Rand|Deg|Rand|sin-1|sinh-1|cos-1|cosh-1|tan-1|tanh-1|'+
'1|1/x|2|x2|3|x3|x√y|√|ln|log2|ex|2x').split('|');if(key===13)key=61;key=String.fromCharCode(key);for(var n=0,m=keyMatch.length;n<m;n=n+2)
{if(key===keyMatch[n])
...
key=key.replace('×','*').replace('÷','/').replace('–','-');if(key!=='='){this.buff[1]=key;}else if(this.buff[0]===false){this.buff[0]=val;}
if(key==='='&&!this.stack[0]&&this.curr&&this.buff[1]){return(this.buff[1]==='yx'?Math.pow(val,this.buff[0]):this.buff[1]==='x√y'?Math.pow(val,1/this.buff[0]):[1]==='EE'?val*Math.pow(10,this.buff[0]):eval('('+val+')'+this.buff[1]+'('+this.buff[0]+')'))+'';}
...
if(rank[key]<=rank[this.stack[this.num-1][1]]){this.stack[this.num-1]=[this.stack[this.num-1][1]==='yx'?Math.pow(this.stack[this.num-1][0],val):this.stack[this.num-1][1]==='x√y'?Math.pow(this.stack[this.num-1][0],1/val):this.stack[this.num-1][1]==='EE'?this.stack[this.num-1][0]*Math.pow(10,val):eval('('+this.stack[this.num-1][0]+')'+this.stack[this.num-1][1]+'('+val+')'),key];}
...
return '0';};keyBoard["0"]="num0_btn";keyBoard["1"]="num1_btn";keyBoard["2"]="num2_btn";keyBoard["3"]="num3_btn";keyBoard["4"]="num4_btn";keyBoard["5"]="num5_btn";keyBoard["6"]="num6_btn";keyBoard["7"]="num7_btn";keyBoard["8"]="num8_btn";keyBoard["9"]="num9_btn";keyBoard["."]="dot_btn";keyBoard["="]="equal_btn";keyBoard["*"]="multiple_btn";keyBoard["×"]="multiple_btn";keyBoard["÷"]="divide_btn";keyBoard["+"]="plus_btn";keyBoard["–"]="minus_btn";keyBoard["/"]="divide_btn";keyBoard["C"]="clear_btn";keyBoard["c"]="clear_btn";keyBoard["CE"]="clear_btn";keyBoard["m-"]="mMinus_btn";keyBoard["m+"]="mPlus_btn";keyBoard["mr"]="mr_btn";keyBoard["m-"]="mMinus_btn";keyBoard["mc"]="mc_btn";keyBoard["+/–"]="plusMinus_btn";keyBoard["√"]="sqrt_btn";keyBoard["%"]="percent_btn";keyBoard["1/x"]="oneDivide_btn";for(var i in keyBoard)
...
if(!bigger&&((!inp||inp==='+/–')&&valAbs!==0)){val=(+val).toPrecision(9);}
tmp=(val+'').split('.');if(tmp[1]){tmp[2]=tmp[1].split('e');if(tmp[2][1]){tmp[1]=tmp[2][0];}
if(!inp||inp==='+/–'){tmp[1]=(((+('1.'+tmp[1])).toPrecision(bigger?14:tmp[2][1]?7:9))+'');if(tmp[1]>=2){tmp[0]=(+tmp[0]+1)+'';}
...
{var key=exportRoot[keyBoard[text]];if(text==='2nd'){secondActive=secondActive?null:true;}else if(text.match(/^[+|÷|×|–|\/|*|yx|x√y|E]+$/)&&text!=='√'){freezeKey(key);}else if(text.match(/^[\d|\.|π]$/)){freezeKey(key,true);exportRoot.c_txt.text="C";}else if(text==='C'){exportRoot.c_txt.text="CE";if(frozenKey)freezeKey(frozenKey);}else if(text.match(/CE|=/)){if(bracketKey)freezeKey(bracketKey,2);freezeKey(key,true);frozenKey=null}else if(text.match(/Deg|Rad/)){radDeg.firstChild.data=deg?'Rad':'Deg';deg=!deg}else if(text==='('){bracketKey=key;freezeKey(bracketKey,2);}else if(text===')'&&brackets===1&&bracketKey){freezeKey(bracketKey,2);}else if(text.match(/^mr$/)&&memory){exportRoot.c_txt.text="C";}
...
function evalKey(key){var dispVal=resBuffer.replace(/\s/g,'').replace(/Error|∞|-∞/,'0')+'',_PI=Math.PI,lastKey;if(!key.match(/2nd|Deg|Rad|m/)){buffStr.push(key);if((buffStr[buffStr.length-2]==='='&&key!=='='&&calculator[brackets].curr)||key==='CE'){buffStr=[key];}}
lastKey=buffStr[buffStr.length-2];if(key.match(/^[\d|\.]$/)||key==='+/–'){if(calculator[brackets].curr&&key!=='+/–'||(key==='+/–'&&lastKey&&lastKey.match(/^[+|–|÷|×|yx|x√y|E|^C]+$/))){dispVal='0';calculator[brackets].curr=false;}
if((Math.abs(+(dispVal+key))>(bigger?1e15:1e9)||dispVal.replace(/^-/,'').length>15||(dispVal.replace('-','').replace(/\./g,'').length>(bigger?14:8))||(dispVal.match(/\.|\e\+/)&&key==='.'))&&key!=='+/–'){buffStr.pop();return;}else if(key==='+/–'){render(!(dispVal.replace(/e[\+|\-]/,'')).match('-')?'-'+dispVal:dispVal.replace(/^-/,''),'+/–');}else{render((dispVal+key).replace(/^(-)*?0(\d)$/,'$1'+'$2'),true);}}else if(key.match(/^C|CE/)){render(calculator[brackets].init(key));}else if(key.match(/^[+|–|÷|×|-|\/|*|yx|x√y|%|E]+$/)&&key!=='√'){render(calculator[brackets].calc(key,dispVal));}else{if(brackets>-1){calculator[brackets].curr='funk';}
...
break;case 'mc':memory=0;break;case 'm+':memory+=+dispVal;break;case 'm-':memory-=+dispVal;break;case 'mr':render(memory+'');break;case '1/x':render((1/dispVal)+'');break;case '√':render(Math.sqrt(dispVal)+'');break;default:break;}}}}